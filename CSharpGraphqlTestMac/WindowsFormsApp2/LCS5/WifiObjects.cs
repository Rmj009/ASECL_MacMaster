...
{
    public enum WiFiType //LCS5 NO 6G
    {
        WiFi,
        WiFi_2G,
        WiFi_5G,
    ...
    public class WiFiCollection
    {
        ...
        public class IperfCollection
        {
            ...
            public IperfCollection(string content, WiFiType wifiType)
            {
                try
                {
                    ...
                    if (wifiType == WiFiType.WiFi_5G)
                    {
                        if (content.Contains("Mbits/sec"))
                            this.bandwidthMbitsSec = Convert.ToDouble(str[5]); // / 1000; to GBytes LCS5
        ...
        {
            //
            //LCS5 ignore rssi_ch_2, rssi_ch_3
            ...
            try
            {
                ...
                if (rssi_ch_0.Count < 3 || rssi_ch_1.Count < 3) //LCS5 ignore || rssi_ch_2.Count < 3 || rssi_ch_3.Count < 3
                {
                    DisplayMsg(LogType.Log, String.Format("RSSI_CH_0 Count:{0}\nRSSI_CH_1 Count:{1}", rssi_ch_0.Count, rssi_ch_1.Count));
                    //LCS5 ignore rssi_ch_2.Count, rssi_ch_3.Count
                    // \nRSSI_CH_2 Count:{2}\nRSSI_CH_3 Count:{3}
                    return false;
                }
